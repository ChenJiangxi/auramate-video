...
**24.8s · 1080×1920 · 真配音 · 全程有声** —— 播放器出不来就
[直接看 mp4](https://cdn.jsdelivr.net/gh/ChenJiangxi/auramate-video@main/docs/demo/demo.mp4)。

![storyboard](docs/demo/storyboard.png)

<sub>每拍取「起 / 终」两帧。另有 [无声 GIF 预览](docs/demo/demo.gif)。</sub>

### 这条 demo 是怎么来的

口播稿说人话，有「我」：

> 输个生日，三秒，出来八个字。我第一次看，一个字也不认识。
> 这八个字不难排，万年历上就有。难的是，它到底在说我什么。

素材是**移动端布局 + 3 倍像素**逐帧录的（2160×3840），镜头走 `locate`：
停一下看清整页 → 走到目标 → 到位定住。字幕按每句配音实测时长算。
...
真正的镜头感来自**停顿**：停一下 → 移过去 → 定住。`--move locate` 就是这个。
另外起终缩放低于 1.25× 一律当没推——`motion.sh` 会直接警告。
