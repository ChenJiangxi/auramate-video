...
**27.7s · 1080×1920 · 真配音 · 全程有声** —— 播放器出不来就
...
**镜头跟着链走**：推近到八个字 → 更近到单个字（「一个字也不认识」）→
拉回八个字（「排它不难」）→ 拉到全页（「谁排都一样」，拉开＝淡化这步）→
转到解读正文。推-拉-推有来有回，不是八拍一个套路。

素材是**移动端布局 + 3 倍像素**逐帧录的（2160×3840），所以推到 2.4× 时输出仍是降采样。

**镜头每拍换个走法**：推 → 横向推进单列 → 反向拉回 → **横扫一整行不缩放** →
向下推到手指点下去 → 向上拉开 → 定住让正文自己往上走。
`lib/check-motion.py` 会拦「每拍都从全屏推进某处」这种同一条路线。
