...
// 渲染安全余量：playwright 的 recordVideo 起止会吃掉一点帧，实测有时比请求的 dur 短
// ~0.1s，卡一短那一拍就黑屏/定格。多录一点，build-vertical 会用 -t 裁到精确时长。
const MARGIN = Number(arg('--margin', '0.6'));
...
(async () => {
  ...
  for (const j of jobs) {
    ...
    await page.waitForTimeout(Math.round((j.dur + MARGIN) * 1000));
    await page.close();
    ...
    if (f) {
      fs.renameSync(path.join(tmp, f), dst);
      okN++;
      // 真量一下，短了就直接说 —— 短卡会让那一拍黑屏，别等 build 才发现
      let got = null;
      try {
        got = parseFloat(execFileSync('ffprobe', ['-v', 'error', '-show_entries',
          'format=duration', '-of', 'csv=p=0', dst]).toString().trim());
      } catch {}
      const warn = got != null && got < j.dur ? `  ⚠ 比这一拍需要的 ${j.dur}s 还短，加大 --margin` : '';
      console.log(`  ✓ ${j.clip} → ${dst}${got != null ? `  (${got.toFixed(2)}s)` : ''}${warn}`);
    }
