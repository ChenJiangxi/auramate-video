...
//        [--desktop]                            # 桌面布局（默认移动端）。见下
//        [--scroll 340] [--scroll-every 3]      # 每隔几帧滚一次、滚多少
//        [--cursor touch|arrow] [--cursor-size 46]   # 画一个看得见的手指/箭头
//        [--no-cursor]                          # 收起手指，但强调层照画
//        [--act actions.json]                   # 动作表：移动/点/打字/滚 + 强调（见 lib/actor.js）
//        [--login-url ... --email ... --pw-env MY_PW]
//        [--click-start]                        # 先点「开始/生成」再等结果
//        [--keep-frames]                        # 保留 PNG 序列
//
// 竖版（默认）走移动端仿真：720×1280 · dsf 3 · iPhone UA · 手指光标。
// 横版 16:9 加 --desktop：1440×900 · dsf 2 · 桌面 UA · 箭头光标（桌面画面里出现
// 一根手指是穿帮的）。不加的话网站会切成移动端布局，录出来根本不是那个界面。
//
// 关于光标：截图里**没有系统光标**。不画的话页面像自己在动、输入框像自己在填字，
// 观众看不出有人在操作。--cursor 注入一个自绘光标，--act 让它真的去点。
// 两者都按帧推进（截一帧的墙上时间不均匀，按时间跑的动画在成片里会忽快忽慢）。
//
// 关于强调：光标只说明「有人在操作」，没说明「该看哪」。动作表里的
// spot（暗场聚光）/ ring（高亮框）/ mark（记号笔）/ label（标注气泡）解决后半句 ——
// 旁白在说哪个字段，画面上就把哪个字段圈出来。同样按帧推进，且每帧重新量目标位置。
//
...
const { Actor, FX_KINDS } = require('./actor');
...
const DESKTOP = has('--desktop');        // 横版 16:9 产品片走这条
const W = Number(arg('--w', DESKTOP ? 1440 : 720));   // CSS 宽度：决定布局，别乱调大
const H = Number(arg('--h', DESKTOP ? 900 : 1280));
// 像素倍数：决定清晰度，推镜靠它。桌面本来就宽，dsf 2 已经是 2880×1800，再高只是慢
const DSF = Number(arg('--dsf', DESKTOP ? 2 : 3));
...
// 桌面画面里出现一根手指是穿帮的，所以默认形态跟着布局走
const DEFCUR = DESKTOP ? 'arrow' : 'touch';
// --cursor 后面可以不跟值（用默认形态）；跟的要是下一个开关就当没写
let CURSOR = null;
if (has('--cursor')) {
  const v = arg('--cursor', DEFCUR);
  CURSOR = (!v || v.startsWith('--')) ? DEFCUR : v;
}
const CURSIZE = Number(arg('--cursor-size', 46));
const ACTFILE = arg('--act');
// 有动作表却看不见光标 = 页面在自己动，等于白做。除非明确 --no-cursor
if (ACTFILE && !CURSOR && !has('--no-cursor')) CURSOR = DEFCUR;
// 高亮（spot/ring/mark/label）画在**同一个叠加层**上。所以有动作表就必须把层装上，
// --no-cursor 只是把手指收起来 —— 不然「只要高亮不要手指」会变成什么都没有。
const OVERLAY_KIND = CURSOR || (ACTFILE ? DEFCUR : null);
let ACTIONS = null;
if (ACTFILE) {
...
(async () => {
  ...
  const ctxOpts = {
    ...
    locale: 'zh-CN',
    // 桌面模式不能开移动端仿真：开了网站就切成移动布局，录出来不是那个界面
    ...(DESKTOP ? {} : { isMobile: true, hasTouch: true, userAgent: MOBILE_UA }),
  };
  ...
  if (OVERLAY_KIND) await installCursor(ctx, { kind: OVERLAY_KIND, size: CURSIZE });
  ...
  if (OVERLAY_KIND) await installCursorNow(page, { kind: OVERLAY_KIND, size: CURSIZE }).catch(() => {});
  ...
  const actor = ACTIONS
    ? new Actor(page, ACTIONS, { fps: FPS, w: W, h: H, cursorVisible: !!CURSOR, log: (m) => console.log(m) })
    : null;
  const nFx = ACTIONS ? ACTIONS.filter((a) => FX_KINDS.includes(a.do)).length : 0;
  console.log(`逐帧截图 ${total} 帧 @ ${W * DSF}×${H * DSF}`
    + `（${DESKTOP ? '桌面' : '移动端'}布局 ${W}×${H}，dsf ${DSF}）`
    + (CURSOR ? ` · 光标 ${CURSOR}` : '') + (actor ? ` · 动作 ${ACTIONS.length} 条` : '')
    + (nFx ? ` · 强调 ${nFx} 处` : ''));
  for (let i = 0; i < total; i++) {
    // 先把这一帧该有的状态摆好，再截。顺序反了就永远慢一帧。
    if (actor) await actor.tick(i);
    else if (OVERLAY_KIND) await page.evaluate(() => window.__cur && window.__cur.ensure()).catch(() => {});
  ...
  // 帧尺寸必须是偶数：dsf 给了小数（比如 1.5 × 810 = 1215）就会出奇数高，
  // libx264 直接报 "Generic error in an external library" —— 而且是**录完才炸**，
  // 前面十几分钟的截图全白等。所以这里无条件把尺寸抹到偶数。
  execFileSync('ffmpeg', ['-nostdin', '-y', '-v', 'error', '-framerate', String(FPS),
    ...
    '-c:v', 'libx264', '-crf', '18', OUT], { stdio: 'inherit' });
