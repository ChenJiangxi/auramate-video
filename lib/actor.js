...
//   {"at":4.0, "do":"wheel", "dy":800, "over":1.6}                滚动（默认缓动，"ease":"linear" 可关）
//   {"at":6.0, "do":"key",   "press":"Enter"}                     按一个键（默认回车，用来提交）
...
class Actor {
  ...
  async _start(a, i) {
    ...
    switch (a.do) {
      ...
      case 'wheel': {
        ...
        // 默认缓动。匀速滚是「机械感」的根源 —— 观众看到的是传送带，不是有人在读。
        // 真人滚页面是「起步快、到位收住」，所以和光标移动用同一条 smoothstep。
        this.wheel = { total: a.dy || 0, done: 0, f0: i, len, ease: a.ease !== 'linear' };
        break;
      }
      case 'key': {
        // 回车提交是网页上最常见的一下操作，没有它就得去猜发送按钮的选择器
        const k = a.press || 'Enter';
        try { await this.page.keyboard.press(k); }
        catch (e) { this.log(`  ! 按键失败 ${k}: ${e.message}`); }
        break;
      }
  ...
  async tick(i) {
    ...
    if (this.wheel) {
      const w = this.wheel;
      const t1 = Math.min(1, (i - w.f0 + 1) / w.len);
      const want = w.total * (w.ease ? smooth(t1) : t1);
      const step = want - w.done;          // 按累计量算增量，四舍五入不会漂
      w.done = want;
      if (Math.abs(step) > 0.01) { try { await this.page.mouse.wheel(0, step); } catch {} }
      if (i - w.f0 >= w.len) this.wheel = null;
    }
