...
| 断行依据 | **渲染像素宽度**（默认 ≤ 画幅 86%） | 中英混排时字数完全不代表宽度：「而 auramate」10 个字符只有 6 个汉字宽 |
| 每行 | 按宽度自动，也可 `--max-chars` 强制按字数 | — |
| 字体 | `/System/Library/Fonts/STHeiti Medium.ttc` | macOS 自带中文 |

### 切分保留标点（别把逗号吃掉）

早先的实现用 `re.split(r'[，。！？、；：]')` 切句，**分隔符被丢掉了**，于是
「排盘差了一整天，等于把这个人，算成了另一个人。」合并成一行后变成：

```
❌  排盘差了一整天等于把这个人      ← 一串没有停顿的字，读者断不开
✅  排盘差了一整天，等于把这个人    ← 行内标点保留，只去掉行尾标点
```

现在用 lookbehind 切分（`(?<=[，。！？、；：])`），标点留在行内，
只把**行尾**那个标点去掉（屏幕上不需要句号）。

另外太短的行（< 5 字）会尝试并进相邻行，免得出现孤零零一行。

### 钩子那句不要合并子句

默认会把短子句合并到「一行放得下」为止（一行读起来更完整）。但**钩子句必须保住三拍节奏**：
...
| 字幕读起来一串没停顿 | 切句时把标点吃掉了 → 现在 lookbehind 切分，行内标点保留 |
| 中英混排的行忽长忽短 | 按字数断行 → 现在按渲染像素宽度断行 |
