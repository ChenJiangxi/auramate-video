...
**结论是「录宽 + 推近」，不是「录窄 + 将就」** —— 这条以前是反的，实测纠正过来：

```
录 720 窄布局 → 内容看着大，但升到 1080 已经 1.5×，再推就糊
              → 于是不敢推 → 全片只有 1.06–1.12× 的位移 → 观众一眼看出「这不是推镜，是平移」
录 1080 宽布局 → 内容看着小、留白多，但可以放心推 1.7-1.8×
              → 推到位之后主体又大又锐
```

同一页内容、同样推到 1.8×：720 源要放大 2.7×（糊），1080 源只要 1.8×（清楚）。
**留白不是问题，推镜会把它裁掉；分辨率不够才是问题，推镜会把它放大。**
...
| `--move` | 干什么 | 什么时候用 |
|---|---|---|
| `punch-in` | 全屏 → 目标框 | **最常用**。先看全貌再把重点顶到脸上 |
