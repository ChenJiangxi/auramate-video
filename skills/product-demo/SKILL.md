...
### 0. 先定画幅：竖版是默认，横版必须 `--desktop`

`lib/rec-frames.js` 默认按**移动端仿真**录（`isMobile` + iPhone UA + 720×1280 + dsf 3），
这是竖版要的。**录 16:9 不加 `--desktop` 就等于录错了** —— 移动端仿真会让网站
切到手机布局，主视觉缩成一条，那不是你要展示的那个界面。

```bash
# 竖版（默认）
node lib/rec-frames.js --url <URL> --out footage/rec/x.mp4 --fps 12 --secs 12
# 横版 16:9
node lib/rec-frames.js --url <URL> --out footage/rec/x.mp4 --desktop --w 1440 --h 810 --dsf 2
```

| | 视口 | dsf | 每帧 | UA | 光标默认 |
|---|---|---|---|---|---|
| 竖版 | 720×1280 | 3 | 2160×3840 | iPhone | `touch` 手指 |
| 横版 `--desktop` | **1440×810** | 2 | 2880×1620 | 桌面 | `arrow` 箭头 |

三条跟着来的规矩：

1. **横版视口自己就得是 16:9。** `--desktop` 的默认值是 1440×900（16:10），
   直接 `scale=1920:1080` 是**横向拉伸**（1.6 → 1.778，脸和圆都会变胖）。
   要 1920×1080 就显式写 `--w 1440 --h 810`，缩下去是纯降采样。
2. **桌面画面里不许出现手指。** 一根手指点桌面 UI 是穿帮的，`--desktop` 默认换成箭头。
3. **强调别落在画面最底下那一行。** 1080 高的成片字幕带在 y≈924–982，
   底部元素的高亮框和标注气泡会被字幕糊住。要圈底部内容，换成整块的 `spot`。
...
**录屏最容易翻车的不是画质，是节奏。** 而节奏不是剪出来的，是**录之前选出来的** ——
先问这个界面有没有能推着画面往前走的东西。只有三种：

| 发动机 | 长什么样 | 典型页面 |
|---|---|---|
| **可滚的长内容** | 一屏放不下，往下滚就一直有新东西 | 报告页、榜单、长文 |
| **内容在生长** | 画面自己在变：流式出字、图表加载、骨架屏换真内容 | AI 回答、生成中 |
| **人在操作** | 打字、点、切 tab、展开 | 表单、对话框 |

**一个都没有就别硬拍。** 真实教训：灵伴（对话型产品）的首页，1920×1080 下对话内容
撑不满一屏 —— 排了三次滚动，录完发现三次全是空的，**28 秒里 18 秒画面一动不动**。
...
有没有"又长又匀速"的传送带 / 全片有没有轻重变化。

四条量出来的规矩：

1. **开头 3 秒定住。** 观众得先认出这是什么画面。一上来就滚 = 还没看清就过去了。
2. **连续滚很久不是错，匀速才是错。** 这条推翻过我自己的假设：
   拿一条 5.5 万赞的量过（`references/douyin-case-formsight.md`），
   它**连续滚了 18 秒**照样好看，因为滚得有快有慢（峰值/均值 1.58）。
   卡的是"又长又平"，不是"长"。
3. **动的秒数占比低于 20% = 基本是张静态图**，观众没有理由留下。
4. **量的时候要数「真的变了的像素占多少」，不是平均像素差。**
   粒子球、呼吸灯这种常驻动画是弥散的小幅抖动，用平均差会被它抬起一个底噪，
   结果「一屏死内容 + 一个动画球」被判成"全程都在动"—— **正好判反**。
   灵伴那个灵体就是这样，第一版度量差点让我把一条死片子判成合格。

两条配套的：

- **fps 跟着内容走。** 静止/慢动作 12fps 够；**有滚动就得 ≥20fps**，
  否则逐帧截图拼出来的滚动是一顿一顿的。
- **内容不动的时候，用强调层制造事件。** `spot` / `ring` 出现和退场本身就是画面变化 ——
  产品没有"生长"可拍时，这是唯一能自己造出来的节拍。

**灵伴这个例子的最终答案**（找了四条录屏才找对）：**别在 `/app` 里找节奏，去落地页。**
`auramate.net/` 是约 4700px 的长页（可滚）+ 一个「唤醒我的灵体」按钮（点击事件），
点进去滚到顶还能接上「点两下唤醒灵体」——灰白小球变成绚丽粒子球，视觉落差最大的一拍。
量出来：/app 里怎么排都只有 10–17% 动占比，落地页这条 41–58%。
**同一个产品，换个页面，节奏从不及格到及格。**

---

### 9. 竖版放桌面录屏：取景要**切**，不是一拍一个框

抖音上跑得动的产品展示片（拆过一条 3.8 万赞的，见 `references/douyin-case-zhishi.md`）是这个形态：
**横版录屏放竖屏中间、上下留黑边、字幕压在下方黑边里** —— 不拉满、不裁两头，
观众一眼知道"这是在看电脑"，16:9 的界面也不会被切掉左右。

但光这样不够，**取景必须一直在换**。拆那条量过（隔 0.5 秒抽帧）：
...
**不是连续推拉，是切。** 平均 2–2.5 秒换一个机位，近景 ↔ 宽景来回换，
密度比配音句子还高。做这类片子踩过的三版：

| 版本 | 做法 | 结果 |
|---|---|---|
| v1 | 整屏缩进条里，不裁 | 桌面 UI 的字小到读不了 |
| v2 | 一拍裁一个固定框 | 字能读了，但 5–8 秒一个框不动 —— 呆板（H13 写着的错） |
| v3 | `motion.sh` 连续推拉 | 更糟：`punch-in`/`kenburns` 都**从全屏起手**，"要看清某个字段"的拍大半段还是小画面 |
| v4 | **一拍拆 2–3 个机位，近远交替** | 对了 |

所以规矩是：

1. **一拍超过 4 秒就拆机位**，每个机位 1.5–2.6 秒。
2. **同一拍里的机位必须近远交替**（比如 ×1.9 特写 → ×1.2 宽景 → ×1.85 换个位置特写），
   两个机位都是近景等于没换。
3. 连续推拉留给「引入」和「揭示」那一两拍（`punch-in` 引入、`pull-out` 揭示），
   别当主力 —— 它们起手都在全屏。
...
> ⚠ **先判断这条片子要不要套壳。** 壳是为了**遮旧域名 / 统一品牌**，不是为了「显得专业」。
> 拆过一条 5.5 万赞的同赛道视频（`references/douyin-case-formsight.md`），
> 画面是 Windows Chrome 原生窗口，标签栏、地址栏、本地 `C:/Users/…` 路径全露着 ——
> 反而像**真的在给你看我电脑上的东西**。
> 判断标准：可信度来自「产品很成熟」还是「人很真诚」？
> 产品官宣 / 功能上新 → 套；「我做了个东西给你看」/ 翻车复盘 / 开发过程 → 别套。
...
| 16:9 成片里 UI 被横向压扁 | 源视口是 16:10（1440×900）却 `scale=1920:1080` | 录制视口就用 16:9：`--desktop --w 1440 --h 810` |
| 录 16:9 出来却是手机布局 | 忘了 `--desktop`，走的是移动端仿真 | 加 `--desktop`（顺带把光标换成箭头） |
| 底部的高亮被字幕糊住 | 强调落点在 y≈924–982 那条字幕带里 | 底部内容改用整块 `spot`，或把落点换到画面中上部 |
| 录屏里全是上一次操作的残留 | 产品是有状态的，上次点出来的对话/报告还在 | 录之前先跑一遍探路脚本看当前页面长什么样，必要时换个干净入口 |
| 给人看的样片发过去"发灰、脏" | 用 GIF 发暗色 UI 的产品片 | GIF 最多 256 色，暗背景 + 渐变 + 细字必然压成一片灰。**发 mp4 文件或 PNG 关键帧**，别发 GIF |
| 加了一拍，成片却短了一截 | 自己写的 build 脚本把拍名写死了（`for b in b1 b2 …`） | 拍名从 `clips.json` 读，别手写。`lib/build-vertical.sh` 本来就是这么做的 |
| 录了十几分钟，最后一步编码炸了 | `--dsf` 给了小数（1.5 × 810 = 1215），高是奇数，libx264 不收 | 已在 `rec-frames.js` 里无条件抹到偶数；`.frames/` 还在的话直接自己 ffmpeg 编，不用重录 |
| 重录一条更清晰的，画面里那个变化没了 | 那个变化是**一次性**的（首次引导 / 空状态 / 灵体从休眠唤醒），第一条录完状态就留住了 | **一次性的画面第一条就按最终参数录**。真要重来先确认状态能不能重置（换账号 / 清 storage / 产品里的"重新生成"） |
